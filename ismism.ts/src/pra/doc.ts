...
export async function soc(
	...
	const s = await soc_r(_id, { ...pid, sec: 1, uid_max: 1, uid: 1, res_max: 1, res: 1 })
	if (!s) return null
...
export async function agd(
	...
	const a = await agd_r(_id, { ...pid, detail: 1, budget: 1, fund: 1, expense: 1, goal: 1, img: 1, res_max: 1 })
	if (!a) return null
	const [unam, nr] = await Promise.all([
