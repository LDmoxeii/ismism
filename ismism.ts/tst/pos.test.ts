...
Deno.test("pre-put", async () => {
	...
	assertEquals([
		...
		pos({ jwt }, "put", json({ put: "soc", soc: 1, agr: "消息" } as Put)),
		await pos({ jwt }, "put", json({ put: "soc", soc: 1, nam: "同城俱乐部", adm1, adm2, sec: [2, 3] } as Put)),
		pos({ jwt }, "put", json({ put: "soc", soc: 1, msg: "消息" } as Put)),
		await pos({ jwt }, "put", json({ put: "soc", soc: 1, agr: "同意" } as Put)),
		pos(p, "put", json({ put: "aut", aut: [2], wsl: [2], lit: [2] } as Put))
	]))
	...
	const rpre = await Promise.all([
		...
		await pos({ jwt }, "pre", json({ pre: "cdt", cdt } as Pre)),
		await pos(p, "pre", json({ pre: "dbt", dbt } as Pre)),
	...
	assertEquals([1, 1, 1, 0, 1, 1, 0, null, 1, 1, 1, 1], await Promise.all([
		...
		pos({ jwt }, "put", json({ put: "agd", agd: 1 } as Put)),
		await pos({ jwt }, "put", json({ put: "cdt", id: cdt._id, agr: 3 } as Put)),
		pos(p, "put", json({ put: "cdt", id: cdt._id } as Put)),
		pos({ jwt }, "put", json({ put: "cdt", id: cdt._id } as Put)),
		pos({ jwt }, "put", json({ put: "cdt", id: cdt._id, agr: 5 } as Put)),
