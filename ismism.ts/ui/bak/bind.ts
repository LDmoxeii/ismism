...
import type { Goal, Tag, Rec, Work, Worker, Fund, Role } from "../../src/dbtyp.ts"
import { UserPass } from "../../src/query/user.ts"

const ver = "ismism-0.0.3-20221220"
let hash = ""
let agenda: Agenda
...
function euserpass(
	...
	const [t, [
		buser,
		fpass, cnbr, ccode_send, bsend, ccode_issue, bissue,
		fsetting, buname, blogout
	]] = template("userpass", [
		"tab.user",
		"form.pass", "nbr", "code.send", "button.send", "code.issue", "button.issue",
		"form.setting", "button.uname", "button.logout"
	])

	if (userpass) {
		buser.innerText = `uid#${userpass.uid}`
		fpass.parentNode?.removeChild(fpass)
	} else {
		fsetting.parentNode?.removeChild(fsetting)
	}
...
async function load(
	...
	await post("userpass_code", { nbr: "11111111111", code: 111111, sms: false })
	userpass = await post("userpass_issue", { nbr: "11111111111", code: 111111, renew: true })
